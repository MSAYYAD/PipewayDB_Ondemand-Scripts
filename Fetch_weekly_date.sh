...
#           will call cronweekly.php script, this is a wrapper program the actual data upload is happening in
#           php script
...
#
#           This script will execute weekly (Only on Sunday)
...
#    echo "i got my current date"
    # calling a php script of changed mode(chmod +x)
    php -f  /var/www/html/cron/cronweekly.php >> /home/pipewaydb/cronweekly_job_error_log.txt   
    break
  else
    endtime=$(date +"%s")
    ...
    if [[ $mail_time -eq 1800 || $mail_time -eq 3600 ||  $mail_time -eq 5400  || $mail_time -eq 7200 ]] ; then
    	sleep 1
    echo "Subject: waiting for weekly trigger from AS400" | sendmailAnwar.hussain@goolean.tech,puja.kumari@goolean.tech,bandana.kumari@goolean.tech nilesh.karanjkar@goolean.tech nitin.kumar@goolean.tech

   #using elif statement to stop infinte loop here.
  elif [[ $mail_time -gt 7200 ]]; then
     echo "Subject: No weekly trigger found for today's date" | sendmail puja.kumari@goolean.tech,bandana.kumari@goolean.tech nilesh.karanjkar@goolean.tech nitin.kumar@goolean.tech
