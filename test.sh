...
do
     ...
     if [[ "$day" == 1  &&   "$count" == 126  &&   "$mail_flag" == 0  &&  "$f2" == $now  ]] || [[  "$day" == 2  &&   "$count" == 134  &&   "$mail_flag" == 0  &&  "$f2" == $now  ]] || [[  "$day" == 3  &&   "$count" == 134  &&   "$mail_flag" == 0  &&  "$f2" == $now  ]] || [[  "$day" == 4  &&   "$count" == 134  &&   "$mail_flag" == 0  &&  "$f2" == $now  ]] || [[  "$day" == 5  &&   "$count" == 134   &&   "$mail_flag" == 0  &&  "$f2" == $now  ]] ; then
...
mysql -h 192.168.13.167 -u admin167 -pAaca@123#  -D aaca_live  -e 'update SP_ERROR_HNDLNG set mail_flag ='1' '
  ...
  if [[ "$day" == 1  &&  "$f1" == 126  &&  "$f2" == $now  && "$count" == 126  ]] || [[  "$day" == 2  &&  "$f1" == 134  &&  "$f2" == $now  && "$count" == 134 ]] || [[  "$day" == 3  &&  "$f1" == 134  &&  "$f2" == $now  && "$count" == 134 ]] || [[  "$day" == 4  &&  "$f1" == 134  &&  "$f2" == $now  && "$count" == 134  ]] || [[  "$day" == 5  &&  "$f1" == 134  &&  "$f2" == $now  && "$count" == 134 ]] ; then
    ...
    if [[ $mail_time -eq 180 || $mail_time -eq 360 ||  $mail_time -eq 540  || $mail_time -eq 720 ]] ; then
   sleep 1
    echo "Subject: waiting for DAILY DATA REFRESHMENT" | sendmail bandana.kumari@goolean.tech deepak.verma@goolean.tech 
   #using elif statement to stop infinte loop here.
  elif [[ $mail_time -gt 720   &&   "$mail_flag" == 0 ]]; then
     echo "Subject: NO DATA REFRESHMENT FOR TODAY" | sendmail bandana.kumari@goolean.tech deepak.verma@goolean.tech 
     exit;
  fi
  if   [[ $mail_time -gt 720   &&   "$mail_flag" == 1 ]]  ; then 
 echo "Subject: DAILY REPORTS ARE OVER FOR TODAY" | sendmail bandana.kumari@goolean.tech deepak.verma@goolean.tech 
 exit;
fi
fi
done
